...
//! Clean-room rust implementation of the specification laied out below:
...
impl Header {
    ...
    #[inline]
    pub const fn with_timestamp(cols: u16, rows: u16, timestamp: i64) -> Self {
...
impl Event {
    ...
    #[inline]
    pub const fn resize(interval: f64, cols: u16, rows: u16) -> Self {
        Self::Resize {
            interval,
            cols,
            rows,
    ...
    #[inline]
    pub const fn exit(interval: f64, status: i32) -> Self {
...
pub fn escape_output_for_json(data: &[u8]) -> String {
    fn push_u16_escape(out: &mut String, value: u32) {
        ...
        out.push('\\');
        out.push('u');
        out.push(char::from(HEX[((value >> 12) & 0xf) as usize]));
        out.push(char::from(HEX[((value >> 8) & 0xf) as usize]));
        out.push(char::from(HEX[((value >> 4) & 0xf) as usize]));
    ...
    while i < data.len() {
        ...
        match b {
            ...
            0x09 => out.push_str("\\t"),
            0x00..=0x1f | 0x7f => {
                push_u16_escape(&mut out, u32::from(b));
            }
            _ => {
                ...
                match std::str::from_utf8(rest) {
                    Ok(s) => {
                        ...
                        if u32::from(ch) <= 0x1F || ch == '\u{7f}' {
                            push_u16_escape(&mut out, u32::from(ch));
                        } else {
                            out.push(ch);
                        }
                        i += ch.len_utf8();
...
pub fn output_event_json_line(interval: f64, data: &[u8]) -> String {
    ...
    let formatted_interval = format_interval(interval);
    format!("[{formatted_interval},\"o\",\"{escaped}\"]\n")
}

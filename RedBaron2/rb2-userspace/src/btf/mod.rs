...
pub fn extract_raw_btf_bytes<'a>(blob: &'a [u8]) -> Result<Cow<'a, [u8]>> {
    if blob.starts_with(b"\x7fELF") {
        let obj = Object::parse(blob).context("failed to parse ELF")?;
        let btf = obj.btf.context("ELF has no .BTF section")?;
        Ok(Cow::Owned(btf.to_bytes()))
    } else {
...
mod tests {
    ...
    fn push_string_table(strings: &[&str]) -> (Vec<u8>, Vec<u32>) {
        let mut buf = vec![0];
        let mut offs = Vec::with_capacity(strings.len());
        for s in strings {

    ...
    if let Ok(entries) = fs::read_dir("/proc") {
        for entry in entries.flatten() {
            if *shutdown_rx.borrow() {
                debug!("Stopping YARA full scan during shutdown");
                break;
            }
...
pub fn yara_init_memory_scan(
    ...
    info!("Yara memory scanning started");
    ...
    loop {
        if *shutdown_rx.borrow() {
            debug!("YARA memory scanning stopping during shutdown");
            return Ok(());
        }
        ...
        // prevent PID-tracked state growing forever
        known_pids.retain(|pid| sys.process(SysPid::from_u32(*pid as u32)).is_some());
        seen_hashes.retain(|pid, _| known_pids.contains(pid));
        ...
        if last_full_scan.elapsed() >= full_scan_interval {
            debug!("Time for full scan of all running processes");

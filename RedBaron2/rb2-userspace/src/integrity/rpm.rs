...
impl StreamScanManager for RpmScanner {
    ...
    fn spawn_producer(
        limits: ScanLimits,
        tx: tokio::sync::mpsc::Sender<StreamItem<Self::Job>>,
    ) -> tokio::task::JoinHandle<anyhow::Result<()>> {
        let db_path = PathBuf::from("/var/lib/rpm/rpmdb.sqlite");
        let scan_conffiles = limits.scan_conffiles;

        tokio::spawn(async move { stream_rpmdb_entries(&db_path, tx, scan_conffiles).await })
    }
...
async fn stream_rpmdb_entries(
    db_path: &Path,
    tx: tokio::sync::mpsc::Sender<StreamItem<VerifyJob>>,
    scan_conffiles: bool,
) -> Result<()> {
    ...
    for row in rows {
        ...
        if tx.send(StreamItem::Pkg).await.is_err() {
            return Ok(());
        }

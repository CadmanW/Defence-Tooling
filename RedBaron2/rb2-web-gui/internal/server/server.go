...
func (s *Server) newMux() *http.ServeMux {
	...
	registerPlayerAssetHandlers(mux, s)
	return mux
}
...
func (s *Server) downloadAndIndex(ctx context.Context, sessionID, host, s3Path string, forceReindex bool) error {
	...
	for _, key := range keys {
		...
		dec, err := decrypt.DecodeChunk(key, raw, s.identity)
		if err != nil {
...
func (s *Server) renderPlayerPage(host, sessionID, sizeHuman string, blobCount int, startFmt, endFmt, s3Path string) (string, error) {
	playerCSSURL, playerJSURL, err := s.playerAssetURLs()
	if err != nil {
		return "", err
	}

	titleSessionID := sessionID
	if len(titleSessionID) > 8 {
		titleSessionID = titleSessionID[:8]
	}

	page := playerHTML
	page = strings.ReplaceAll(page, "{{TITLE}}", html.EscapeString(host+" / "+titleSessionID))
	...
	page = strings.ReplaceAll(page, "{{SIZE}}", html.EscapeString(sizeHuman))
	page = strings.ReplaceAll(page, "{{BLOBS}}", fmt.Sprintf("%d", blobCount))
	page = strings.ReplaceAll(page, "{{START}}", html.EscapeString(startFmt))
	page = strings.ReplaceAll(page, "{{END}}", html.EscapeString(endFmt))
	page = strings.ReplaceAll(page, "{{S3_PATH}}", s3Path)
	page = strings.ReplaceAll(page, "{{PLAYER_CSS_URL}}", html.EscapeString(playerCSSURL))
	page = strings.ReplaceAll(page, "{{PLAYER_JS_URL}}", html.EscapeString(playerJSURL))
	return page, nil
}

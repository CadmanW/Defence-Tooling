...
/**
 * Modified by the red baron 2 developers
 * Original Source:
 * https://github.com/elastic/ebpf/blob/main/GPL/Events/Process/Probe.bpf.c
 * https://github.com/elastic/ebpf/blob/main/GPL/Events/EbpfEventProto.h
 * https://github.com/elastic/ebpf/blob/main/GPL/Events/Varlen.h
 * https://github.com/elastic/ebpf/blob/main/GPL/Events/Helpers.h
 */
...
static void ebpf_pid_info__fill(struct ebpf_pid_info *pi,
  ...
  */
  ...
  u32 nr = 0;
  const struct upid *u =
      (const struct upid *)((const char *)spid +
                            bpf_core_field_offset(struct pid, numbers));
  ...
  */
  ...
  // audit loginid sessionid values
  pi->audit_loginuid = BPF_CORE_READ(task, loginuid.val);
  pi->audit_sessionid = BPF_CORE_READ(task, sessionid);
}
